...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-6">
            ...
            <nav className="flex items-center gap-1">
              {scheduleId !== null ? (
                <div className="flex items-center gap-2 text-sm px-1 mr-1">
                  <button
                    onClick={() => { setScheduleId(null); fetchConferences() }}
                  ...
                  <span className="font-medium text-gray-900 truncate max-w-[200px]">
                    {conferences.find((c) => c.id === scheduleId)?.name ?? ''}
                  </span>
                </div>
              ) : (
                <button
              ...
              <button
                onClick={() => { setScheduleId(null); setActiveTab('logs'); fetchConferences() }}
                className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${
                  activeTab === 'logs' && scheduleId === null
                    ? 'bg-blue-50 text-blue-700'
                    : 'text-gray-500 hover:text-gray-900 hover:bg-gray-100'
                }`}
              >
              ...
              <button
                onClick={() => { setScheduleId(null); setActiveTab('settings') }}
                className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${
                  activeTab === 'settings' && scheduleId === null
                    ? 'bg-blue-50 text-blue-700'
                    : 'text-gray-500 hover:text-gray-900 hover:bg-gray-100'
                }`}
              >
          ...
          <div className="flex items-center gap-3">
            {scheduleId === null && activeTab === 'conferences' && !editing && (
              <button
                onClick={openCreate}
      ...
      {scheduleId !== null ? (
        <SchedulePage conferenceId={scheduleId} />
      ) : activeTab === 'settings' ? (
